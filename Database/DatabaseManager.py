...
class DataBase:
    ...
    def createAccount(self):
        ...
        data = {
            ...
            "upgradeTokens": self.player.upgradeTokens,
            "coins_reward": 0,
    ...
    def getRoomId(self):
        ...
        if max_id is None:  
            self.player.teamID = 1
        else:
            self.player.teamID = max_id + 1
    ...
    def updateGameroomInfo(self, value, roomId, index):
        ...
        if data:
            ...
            updateQuery = "UPDATE Gamerooms SET data = ? WHERE room_id = ?"
            self.executeQuery(updateQuery, [json.dumps(gameroomData), roomId])
    def getGameroomInfo(self, index, ID=None):
        if ID == None:
            ID = self.player.teamID
        self.cursor.execute("SELECT data FROM Gamerooms WHERE room_id = ?", (ID,))
        result = self.cursor.fetchone()
        if result:
          gameroomData = json.loads(result[0])
    ...
    def onlineMembers(self, clubId, amount):
        query = "SELECT data FROM Clubs WHERE club_id = ?"
        data = self.fetchOne(query, [clubId])
        if data:
            clubData = json.loads(data[0])
            clubData["info"]["onlineMembers"] += amount
            updateQuery = "UPDATE CLubs SET data = ? WHERE club_id = ?"
    ...
    def rerollEvents(self):
        ...
        current_state = json.loads(row1[0]) if row1 else {"info": {"events": {}}}

        self.cursor.execute("SELECT data FROM Events WHERE state = 2")
        row2 = self.cursor.fetchone()
        next_state = json.loads(row2[0]) if row2 else {"info": {"events": {}}}

        current_events = current_state["info"]["events"]
        next_events = next_state["info"]["events"]

        updated = False

        for key in refresh_hours:
            if key in current_events and "TimeStamp" in current_events[key]:
                if now.timestamp() >= current_events[key]["TimeStamp"]:
                    if key in next_events:
                        event_data = next_events[key]
                    else:
                        next_refresh = now + timedelta(hours=refresh_hours[key])
                        current_events[key] = {
                        "ID": event_data["ID"],
                        "Status": 2,
                        "TimeStamp": int(next_refresh.timestamp()),
                        "Tokens": 10
                        }
                    ...
                    future_refresh_time = next_refresh+ timedelta(hours=refresh_hours[key])
                    next_events[key] = {
                        "ID": random.choice(Locations().getAllLocationsWithException(["Survival", "BossFight"])),
                        "Status": 2,
                        "TimeStamp": int(future_refresh_time.timestamp()),
                        "Tokens": 10
                    }
        ...
        if updated:
            self.cursor.execute("UPDATE Events SET data = ? WHERE state = 1", [json.dumps(current_state)])
            self.cursor.execute("UPDATE Events SET data = ? WHERE state = 2", [json.dumps(next_state)])
            self.connection.commit()
    ...
    def createBattle(self):
        ...
        BrawlersList = Characters().getBrawlers()
        for x in range(5):
        ...
        areaEffects = {str(i): copy.deepcopy({"objectInfos": {"x": 2550, "y": 4950, "index": i+102, "z": 0, "visibility": 10}}) for i in range(0)}
        gameObjectsArray = {"count": 7, "csvIDArray": {str(i): copy.deepcopy({"classID": 16, "instanceID": csvid[i-1]}) for i in range(1, 8)}, "indexArray": {str(i): copy.deepcopy({"classID": 2, "instanceID": i-1}) for i in range(1, 8)}, "gameObjects": {"heroes": heroes, "projectiles": projectiles, "areaEffects": areaEffects, "characters": characters, "items": items}}
        data = {"globalID": 2000000, "fadeCounter": 0, "isGameOver": False, "unknownBoolean": True, "unknownBoolean1": True, "unknownBoolean2": True, "unknownBoolean3": True, "unkMapSize": 0, "unkMapSize2": 0, "tileMapWidth": 0, "tileMapHeight": 0, "destructibleTiles": destructibleTiles, "ultiArray": ultiArray, "progressionSelf": 0, "progressionRival": 0, "killArray": killArray, "gameObjects": gameObjectsArray}
